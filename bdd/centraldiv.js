...
async function createCentralDivTable() {
  const client = await pool.connect();

  try {
    // Créez la table centraldiv si elle n'existe pas déjà
    await client.query(`
      CREATE TABLE IF NOT EXISTS centraldiv(
    ...
    console.error('Erreur lors de la création de la table centraldiv:', error);
  } finally {
    client.release();
  }
}
...
async function insertData() {
  ...
  try {
    for (let i = 1; i <= 12; i++) {
      const query = `
        INSERT INTO centraldiv(e1, e2, e3, e4, e5, e6, e7, e8, e9, e10, e11, e12, e13, e14, e15, e16, e17, e18, e19, e20, e21, e22)

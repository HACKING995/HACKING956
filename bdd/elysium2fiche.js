...
async function createElysium2FicheTable() {
  ...
  try {
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS elysium2fiche(
    ...
    `);
    console.log('Table elysium2fiche créée avec succès');
  } catch (error) {
    console.error('Erreur lors de la création de la table elysium2fiche:', error);
  } finally {
    client.release();
  }
}
...
async function insertData() {
  ...
  try {
    for (let i = 1; i <= 30; i++) {
      const query = `
        INSERT INTO elysium2fiche(e1, e2, e3, e4, e5, e6, e7, e8, e9, e10, e11, e12, e13, e14)
...
async function getData(ide) {
  ...
  try {
   const query = 'SELECT e1, e2, e3, e4, e5, e6, e7, e8, e9, e10, e11, e12, e13, e14 FROM elysium2fiche WHERE id = $1';
